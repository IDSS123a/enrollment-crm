  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (isLogin) {
        ...
        if (error) {
          toast({ title: 'Error', description: error.message || t('loginError'), variant: 'destructive' });
        } else {
          navigate('/dashboard');
        }
      } else {
      ...
      <div className="w-full max-w-md">
        <div className="bg-card rounded-2xl shadow-material-xl p-8 animate-fade-in">
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            {!isLogin && (
              <div className="space-y-2">
                ...
                <div className="relative">
                  ...
                  <Input id="fullName" type="text" value={fullName} onChange={(e) => setFullName(e.target.value)} className="pl-10" placeholder="John Doe" required={!isLogin} />
                </div>
              </div>
            )}
            ...
            <Button type="submit" className="w-full gradient-primary hover:opacity-90 text-white font-medium py-6" disabled={loading}>
              {loading ? (
          ...
          <div className="mt-6 text-center">
            <button type="button" onClick={() => setIsLogin(!isLogin)} className="text-sm text-primary hover:underline">
              ...
              <span className="font-semibold">{isLogin ? t('signup') : t('login')}</span>
            </button>
          </div>

          <p className="mt-6 text-xs text-center text-muted-foreground">{t('gdprNotice')}</p>
        </div>
      </div>
    </div>
  );
